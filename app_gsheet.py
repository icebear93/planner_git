...
def load_config():
    ...
    for k, v in DEFAULT_CONFIG.items():
        cfg.setdefault(k, v)
    cfg["auto_phase"] = bool(cfg.get("auto_phase", True))
    cfg["manual_phase"] = int(cfg.get("manual_phase", 1))
...
def load_subjects():
    ...
    # 타입 보정
    for r in rows:
        r["total_lectures"] = int(r.get("total_lectures", 0))
        r["completed_lectures"] = int(r.get("completed_lectures", 0))
...
with st.sidebar:
    ...
    day_type = st.selectbox(
        ...
        index=list(DAY_TYPE_LABELS.keys()).index(get_day_type(selected_date)),
        format_func=lambda x: DAY_TYPE_LABELS[x],
    )

    if config["auto_phase"]:
    ...
    mode = st.radio("모드", list(MODE_LABELS.keys()), format_func=lambda x: MODE_LABELS[x], horizontal=True)
...
tab_dashboard, tab_routine, tab_subjects, tab_analysis = st.tabs(
    ["🏠 대시보드", "✅ 오늘 루틴", "📚 과목 관리", "📊 분석"]
)
